...
%DIF ADD drones-4134001-done.tex                                     Fri Feb 27 11:00:22 2026
...
}\DIFdelend \DIFaddbegin \DIFadd{Agisoft Metashape version 1.6.4 (Agisoft LLC., St. Petersburg, Russia), for orthorectiphication }\DIFaddend . Together with the \DIFdelbegin \DIFdel{orthorectified }\DIFdelend \DIFaddbegin \DIFadd{orthomosaic }\DIFaddend image (typically in the .tif format), a digital elevation model (DEM) file is also produced, which describes the three-dimensional structure of the field.
...
\DIFmodbegin
\begin{lstlisting}[caption={Example of configuration .ini file to run drone2report},label={lst:config},alsolanguage=DIFcode]
[DEFAULT]
...
[TASK indices]
%DIF > #this section tells DRONE2REPORT to compute two indices: GLI and HUE
active=True
...
\DIFaddbegin \DIFadd{We emphasize that the presented case studies are not original research work, but mere examples of possible uses of }\textsc{\DIFadd{drone2report}}\DIFadd{.
}\DIFaddend 
...
%    \centering
    ...
    \caption{Example .dem file of a barley field (left: flight on 19/04/2024); plant height (metres) calculated from the .dem file by drone2report, over 10 flights covering the growing season (right\DIFaddbeginFL \DIFaddFL{; the dates fo the 10 flights are the same as in Figure~\ref{fig:time-curves}}\DIFaddendFL ).}
...
The indices obtained with this case study are likely overfitting, at least partially. A focused, future study, could estimate the fraction of overfitting and apply regularization techniques to ensure good generalization of the results.
...
\textit{QGIS} (\url{https://qgis.org/}, \cite{moyroud2018introduction}) is a free and open-source desktop geographic information system (GIS) suite. It allows users to create, edit, visualize, analyze, and publish geospatial information. In particular, QGIS provides a Processing Modeler \cite{dobesova2020evaluation} for chaining algorithms and running them in batch, which can partially address reproducibility. In practice, model portability, parameter/version management, and experiment-level reporting remain challenging, especially across groups and compute environments.
