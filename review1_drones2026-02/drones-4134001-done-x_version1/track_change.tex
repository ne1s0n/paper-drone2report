...
%DIF ADD drones-4134001-done.tex                                     Wed Mar  4 12:48:38 2026
...
To illustrate the removal of background noise, we use one RGB image of tobacco leaves from the Github repository (\url{https://github.com/tanghaibao/jcvi/wiki/GRABSEEDS}) of the \textsc{GRABSEED} software \cite{tang2024grabseeds} (\DIFdelbegin \DIFdel{see }\DIFdelend Figure~\ref{fig:tobacco-leaves})
...
As }\DIFaddend vegetation indices, we use GLI (described before, see Formula~\ref{formula:GLI_index}) and HUE~\cite{mathieu1998relationships}\DIFaddbegin \DIFadd{, }\DIFaddend defined as:
...
where $\textbf{p} \in \mathbb{R}^{14}$ is a pixel on the merged 14-bands image. The set of 30 coefficients $[n_0, ..., n_{14}, d_0, ..., d_{14}]$ are to be chosen to maximize the Pearson's correlation between the array of \DIFdelbegin \DIFdel{heading dates and the array of }\DIFdelend per-plot average index values \DIFaddbegin \DIFadd{and the target variable (heading dates, collected in a 264-cells array)}\DIFaddend . Formula~\ref{optimal_index} is designed as a generalization of the most commonly used indices\DIFaddbegin \DIFadd{. In fact, indices }\DIFaddend such as NDVI, GLI \DIFdelbegin \DIFdel{, VARI , etc.\mbox{%DIFAUXCMD
...
This thus becomes a 30-parameters optimization problem, which can be solved via a new custom \textsc{drone2report} task. Internally, the task uses \texttt{Scipy}'s \cite{2020SciPy-NMeth} \textit{optimize.minimize()} function based on the \textit{Nelder-Mead} numerical method \cite{nelder1965simplex}. The results of the optimization process are reported in Figure~\ref{fig:optimization_trajectory}, which shows the optimization trajectory of the custom index in equation~\ref{optimal_index} through all training iterations. The index was optimized on the merged 14-bands image and, for comparison, on both the visible RGB image and the 10-bands multispectral image, separately. \DIFaddbegin \DIFadd{For reference, on the 3-bands RGB image the number of coefficients becomes eight, on the multispectral image it becomes 22.
}\DIFaddend Additionally, the plot also reports the Pearson's correlation with heading date for the two best performing vegetation indices among the twenty one currently implemented in \textsc{drone2report}, namely NDVI (Normalized Difference Vegetation Index, for multispectral images) and GLI (Green Leaf Index, for RGB images).

\begin{figure}[H]
%    \centering
    \includegraphics[width=1\linewidth]{figs/trajectories.png}
    \caption{Optimization trajectories through training cycles. The custom index described in Formula~\ref{optimal_index} was optimized for maximal Pearson's correlation between the vector of its per-plot averages and the phenotype \textit{heading date} \DIFaddbeginFL \DIFaddFL{(here acting as target variable)}\DIFaddendFL . Solid lines show three optimization trajectories on different input images, namely RGB, 10-bands multispectral, and a 14-bands merged image containing RGB, multispectral and temperature data. For reference, the two best performing indices (NDVI and GLI) are also reported.
...
\DIFaddend A focused, future study, could estimate the fraction of overfitting and apply regularization techniques to ensure good generalization of the results.
