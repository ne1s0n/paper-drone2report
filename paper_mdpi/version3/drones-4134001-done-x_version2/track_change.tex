...
%DIF DEL ../drones-4134001-done-x_version1/drones-4134001-done.tex   Thu Mar  5 16:10:34 2026
%DIF ADD drones-4134001-done.tex                                     Thu Mar 19 11:09:53 2026
...
Orthophotos are typically multichannel images, with common cases being three channels (visible RGB spectrum), while multispectral images may include additional bands, such as near-infrared (NIR). Thermal data is encoded as a single channel and possibly rendered as grayscale images, with each pixel corresponding to a single temperature read. Even elevation models (DEM files) can at this stage be considered as a valid data input: a single channel image where each pixel corresponds to a measured height. \DIFaddbegin \DIFadd{The number of channels is typically maintained when orthophotos are then combined in orthomosaics.
}\DIFaddend 

\textsc{drone2report} can receive as input one or more \DIFdelbegin \DIFdel{orthophotos}\DIFdelend \DIFaddbegin \DIFadd{orthomosaics}\DIFaddend . If more than one image is provided, they will be internally stacked into a new multichannel image called \textit{dataset}. For example, providing an RGB image plus a thermal image will create a four-channels RGBT \textit{dataset}. Differences in image sizes and resolutions are solved via \DIFaddbegin \DIFadd{cubic }\DIFaddend interpolation and taking into account possible differences in georeferencing data. This results in the creation of a single tensor-like data structure with a single shape (width and length) and as many channels as the sum of all input images channels. This unified representation enables to perform algebraic operations on all combined channels simultaneously, and even combine data coming from different flights and sensors. Even when multiple \DIFdelbegin \DIFdel{orthophotos }\DIFdelend \DIFaddbegin \DIFadd{orthomosaics }\DIFaddend are combined into a \textit{dataset} the ROIs are specified only once.

One \textit{dataset} can thus be composed of several \DIFdelbegin \DIFdel{orthophotos}\DIFdelend \DIFaddbegin \DIFadd{orthomosaics}\DIFaddend , and several \textit{dataset}s can be loaded. \textsc{drone2report} then proceeds to feed all the data to a queue of tasks. As of the current implementation, a number of tasks is implemented for computing various vegetation indices, rescaling and extracting subimages, and preparing data for successive elaboration (\textit{e. g.} training neural networks). 

\DIFaddbegin \DIFadd{After all tasks have run, }\textsc{\DIFadd{drone2report}} \DIFadd{will run the rendering queue. "Render" items are similar to task but are designed to collect results of other analyses and produce summary and synthetic information. One of the render task is called "report" and is configured to collect all the produced log files, together with 
}
...
%DIF < outfolder=</output/folder>
%DIF > outfolder=</folder/for_analysis_results>
%DIF > logfolder=</folder/for_log_files>
cores=4
...
%DIF > 
%DIF > [RENDER report]
%DIF > #this section details the output html report
%DIF > active=True
%DIF > report_file=${DEFAULT:outfolder}/d2r.html
%DIF > title=Drone2Report analysis
%DIF > author=Your name
%DIF > #all .csv files in this folder, if generated by
%DIF > #index task, will be loaded and plotted
%DIF > index_folder=${DEFAULT:outfolder}/<results>
\end{lstlisting}
\DIFmodend
...
In this section, we briefly survey the most common open-source and/or freeware tools that are currently available for photogrammetric data processing, and describe where \textsc{drone2report} stands in this landscape. \DIFaddbegin \DIFadd{We provide a summary table as Supplementary Table 1.
}\DIFaddend The majority of open-source/freeware software packages for UAV-imagery processing is written in \texttt{Python} and based on libraries for image manipulation like \texttt{scikit-image}, \texttt{PIL/Pillow}, \texttt{GDAL}, \texttt{geopandas}.
...
\DIFdelbegin %DIFDELCMD < \supplementary{ }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \supplementary{
