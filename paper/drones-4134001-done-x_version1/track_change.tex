...
%DIF DEL ../drones-4134001-done-x_version0/drones-4134001-done.tex   Wed Feb 25 10:13:52 2026
%DIF ADD drones-4134001-done.tex                                     Thu Mar  5 16:01:40 2026
...
\noindent\vspace{3pt}\\
%DIF 197c197
%DIF < \textbf{What are the main fundings?}
%DIF -------
\textbf{What are the main findings?} %DIF > 
%DIF -------
...
\begin{itemize}[labelsep=2.5mm,topsep=-3pt]
%DIF 204c204
%DIF < \item A new dedicated library, DRONE2REPORT, is released as a standardized and customizable open-source  framework for processing UAV images in agriculture.
%DIF -------
\item A new dedicated Python library, DRONE2REPORT, is released as a standardized and customizable open-source framework for processing UAV images in agriculture. %DIF > 
%DIF -------
...
%DIF 212-213c212-217
%DIF < \abstract{\textbf{Background}: Unmanned aerial vehicles (UAVs) have become indispensable tools in precision agriculture and plant phenotyping, enabling rapid, non-destructive assessment of crop traits across space and time. Equipped with RGB, multispectral, thermal, and other sensors, UAVs provide detailed information on canopy structure, physiology, and stress responses that can guide management decisions and accelerate breeding programs. Despite these advances, the downstream processing of UAV imagery remains technically demanding. Converting orthophotos into standardized, biologically meaningful data often requires a combination of photogrammetry, geospatial analysis, and custom scripting, which can limit reproducibility and accessibility across research groups. \textbf{Results}: We present \textsc{drone2report}, an open-source python-based software that processes orthophotos from UAV flights to generate vegetation indices, summary statistics, and visual reports, supporting both research and applied breeding contexts. Alongside the basic structure and functioning of \textsc{drone2report}, we present also five case studies that illustrate practical applications common in UAV/drone-phenotyping of plants: i) thresholding to remove background noise and highlight regions of interest; ii) monitoring plant phenotypes over time; iii) extracting information on plant height to detect events like lodging; iv) integrating multiple sensors (cameras) to construct and optimise new synthetic indices; v) integrate a trained deep learning network to implement a classification task.
%DIF < These examples demonstrate the tool’s ability to automate analysis, integrate heterogeneous data and models, and support reproducible computation of agronomically relevant traits. \textbf{Conclusions}: \textsc{drone2report}  streamlines UAV image processing for precision agriculture by linking orthophoto data to standardized, plot-level outputs. Its modular, configuration-driven design allows transparent workflows, easy customization, and integration of multiple sensors within a unified analytical framework. By facilitating reproducible, multi-modal image analysis, drone2report lowers technical barriers to UAV-based phenotyping and opens the way to robust, data-driven crop monitoring and breeding applications.
%DIF -------
\abstract{Unmanned aerial vehicles (UAVs) have become indispensable tools in precision agriculture and plant phenotyping, enabling rapid, non-destructive assessment of crop traits across space and time. Equipped with RGB, multispectral, thermal, and other sensors, UAVs provide detailed information on canopy structure, physiology, and stress responses that can guide management decisions and accelerate breeding programs. Despite these advances, the downstream processing of UAV imagery remains technically demanding. Converting orthophotos into standardized, biologically meaningful data often requires a combination of photogrammetry, geospatial analysis, and custom scripting, which can limit reproducibility and accessibility across research groups. %DIF > 
 %DIF > 
We present \textsc{drone2report}, an open-source python-based software that processes orthophotos/orthomosaics from UAV flights to generate vegetation indices, summary statistics and derived subimages, supporting both research and applied breeding contexts. Alongside the basic structure and functioning of \textsc{drone2report}, we present also five case studies that illustrate practical applications common in UAV/drone-phenotyping of plants: i) thresholding to remove background noise and highlight regions of interest; ii) monitoring plant phenotypes over time; iii) extracting information on plant height to detect events like lodging or the falling over of spikes; iv) integrating multiple sensors (cameras) to construct and optimise new synthetic indices; v) integrate a trained deep learning network to implement a classification task. %DIF > 
These examples demonstrate the tool’s ability to automate analysis, integrate heterogeneous data and models, and support reproducible computation of agronomically relevant traits. %DIF > 
 %DIF > 
\textsc{drone2report}  streamlines UAV image processing for precision agriculture by linking orthophoto data to standardized, plot-level outputs. Its modular, configuration-driven design allows transparent workflows, easy customization, and integration of multiple sensors within a unified analytical framework. By facilitating reproducible, multi-modal image analysis, \textsc{drone2report} lowers technical barriers to UAV-based phenotyping and opens the way to robust, data-driven crop monitoring and breeding applications. %DIF > 
%DIF -------
}
...
\label{intro}
Advances in precision agriculture and plant phenotyping increasingly rely on high-throughput, non-destructive methods for measuring crop performance under field conditions. Remote sensing platforms have become central to this effort, providing quantitative information on canopy structure, physiology, and stress responses. Among \DIFdelbegin \DIFdel{them}\DIFdelend \DIFaddbegin \DIFadd{these}\DIFaddend , unmanned aerial vehicles (UAVs, or drones) are especially attractive due to their flexibility, affordability, and ability to generate repeated observations at plot or field scale. UAV-mounted sensors—ranging from RGB cameras to multispectral, thermal, and LIDAR (Laser Imaging, Detection \DIFdelbegin \DIFdel{, }\DIFdelend And Ranging) units—enable detailed measurement of vegetation indices, canopy temperature \DIFdelbegin \DIFdel{, }\DIFdelend and topography that can inform breeding programs and management decisions \cite{machwitz2021bridging}.

Although the potential of UAV imagery for plant science is well recognized, the software ecosystem supporting its analysis remains fragmented. Many researchers rely on commercial photogrammetry suites for orthomosaic production, followed by manual processing in GIS software or custom scripts for index calculation and region-of-interest (ROI) extraction. This workflow can be labor-intensive, error-prone, and difficult to reproduce across experiments or research groups. Existing open-source solutions are usually not designed for agriculture applications (\DIFdelbegin \DIFdel{e.g. }\DIFdelend \cite{moyroud2018introduction}), have sometimes very specific targets (e.g. seeds \cite{tang2024grabseeds}, berries \cite{loarca2024berryportraits}), focus on single data types (e.g. RGB or multispectral), provide limited support for multi-modal datasets, or lack transparent configuration and automation mechanisms. 
\DIFaddbegin 

\DIFaddend As UAV datasets grow in size and complexity, there is a growing need for tools that combine reproducibility, flexibility, and ease of use, while remaining accessible to plant researchers without advanced programming expertise.
...
\DIFdelbegin \DIFdel{The software is released under open-source license and readily available at }%DIFDELCMD < \url{https://github.com/ne1s0n/drone2report}%%%
\DIFdel{. 
}\DIFdelend %DIF > The software is released under open-source license and readily available at \url{https://github.com/ne1s0n/drone2report}. 
In this paper, we present the technical details of this new tool, followed by \DIFdelbegin \DIFdel{four }\DIFdelend \DIFaddbegin \DIFadd{five }\DIFaddend case-studies describing examples of its application, a comparison with other available software packages, and a discussion of the merits, limitations and future expansions of \textsc{drone2report}.
...
\DIFaddbegin \DIFadd{We adopt here the following definitions: orthophotos (or orthorectified images) are single drone-captured images that have been geometrically corrected (perspective, camera tilt etc.), while orthomosaics are collection of orthophotos stitched together (taking care of overlaps etc.) \mbox{%DIFAUXCMD
\cite{park2022deep}}\hspace{0pt}%DIFAUXCMD
.
}\DIFaddend The first step is data acquisition, with the drone(s) flying over the field of interest and collecting a series of overlapping images, which are then collated together into \DIFdelbegin \DIFdel{what is called an orthophoto (or orthomosaic, or orthorectified image)}\DIFdelend \DIFaddbegin \DIFadd{an orthomosaic}\DIFaddend . This computational step produces a georeferenced composite image and corrects for several technical issues such as perspective distortion, map projection, and lens aberrations.
The generation of orthophotos \DIFaddbegin \DIFadd{and orthomosaics }\DIFaddend is now considered a well-established process, thanks to the availability of mature open-source tools such as OpenDroneMap \cite{noauthor_welcome_nodate}, MicMac \cite{rupnik2017micmac}, and \DIFdelbegin \DIFdel{QGIS \mbox{%DIFAUXCMD
\cite{moyroud2018introduction}}\hspace{0pt}%DIFAUXCMD
. 
}\DIFdelend \DIFaddbegin \DIFadd{COLMAP \mbox{%DIFAUXCMD
\cite{schoenberger2016sfm}}\hspace{0pt}%DIFAUXCMD
. 
%DIF > and Agisoft Metashape version 1.6.4 (Agisoft LLC., St. Petersburg, Russia), for orthorectiphication . 
}\DIFaddend Together with the \DIFdelbegin \DIFdel{orthorectified }\DIFdelend \DIFaddbegin \DIFadd{orthomosaic }\DIFaddend image (typically in the .tif format), a digital elevation model (DEM) file is also produced, which describes the three-dimensional structure of the field.

\begin{figure}[H]
%    \centering
    \includegraphics[width=0.9\linewidth]{figs/workflow.png}
    \caption{General application workflow. Panel a: drones capture raw, partial images of the field (depending on the mounted sensors). All images from a single flight are collated in a unique \DIFdelbeginFL \DIFdelFL{orthophoto}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{orthomosaic}\DIFaddendFL .  Panel b: \DIFdelbeginFL \DIFdelFL{orthophotos }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{orthomosaics }\DIFaddendFL coming from one or more sensors, together with a shape file that specifies the ROIs, enter \textsc{drone2report} and are converted to an internal \textit{dataset} object. Panel c: all datasets are inputed, in turn, to all queued tasks, each task producing its specific output (e.g. tables, other images etc.)}
    \label{fig:workflow}
\end{figure}
...
One \textit{dataset} can thus be composed of \DIFdelbegin \DIFdel{serveral }\DIFdelend \DIFaddbegin \DIFadd{several }\DIFaddend orthophotos, and several \textit{dataset}s can be loaded. \textsc{drone2report} then proceeds to feed all the data to a queue of tasks. As of the current implementation, a number of tasks is implemented for computing various vegetation indices, rescaling and extracting subimages, and preparing data for successive elaboration (\textit{e. g.} training neural networks). 

\subsection{Software Implementation}

\textsc{drone2report} is a Python-based, open source, free software. It is written to be integrated in standard elaboration pipelines aimed \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend agricultural drone images, both for research and industrial applications. As such, it has no GUI and runs, ideally server-side, on data files and configuration (.ini) files. It has an extensive logging functionality and can be used out of the box without any python coding. However, it is designed with modularity in mind, allowing for straightforward extension through the addition of new tasks.

An example configuration (.ini) file is provided in \DIFdelbegin \DIFdel{Box}\DIFdelend \DIFaddbegin \DIFadd{Listing}\DIFaddend ~\ref{lst:config}, demonstrating how to compute two vegetation indices from a single RGB image.
...
A more powerful intervention consists of designing a new TASK, which will then require its own section in the .ini file. For that purpose, a TASK template is provided \DIFaddbegin \DIFadd{in the documentation of the tool}\DIFaddend . Finally, new types of DATA could be supported, which would require some care to curate the exposed functionalities.
...
The \textsc{drone2report} open-source code is available on-line \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{at }\DIFaddend the following Github repository: \url{https://github.com/ne1s0n/drone2report}.

\begin{minipage}{\hsize}%
\DIFdelbegin %DIFDELCMD < \lstset{frame=single,framexleftmargin=-1pt,framexrightmargin=-17pt,framesep=12pt,linewidth=0.98\textwidth,language=python}%%%
%DIF <  Set your language (you can change the language for each code-block optionally)
%DIFDELCMD < 

%DIFDELCMD < \begin{lstlisting}[caption={Example of function computing the GLI vegetation index on a passed image},label={lst:GLI_function}]
%DIFDELCMD < %%%
%DIFAUXCMD NEXT
\DIFmodbegin
\begin{lstlisting}[caption={Example of function computing the GLI vegetation index on a passed image},label={lst:GLI_function},alsolanguage=DIFcode]
%DIF < def GLI(img, channels):
%DIF <   """Green leaf index, uses red, green, blue channels"""
%DIF <   try:
%DIF <     red   = img[:,:,channels.index('red')]
%DIF <     green = img[:,:,channels.index('green')]
%DIF <     blue  = img[:,:,channels.index('blue')]
%DIF <    -
%DIF <   except ValueError:
%DIF <     return np.nan
%DIF <     -
%DIF <   return(
%DIF <     (2.0*green - red - blue) / 
%DIF <     (2.0*green + red + blue)
%DIF <   ) 
\end{lstlisting}
\DIFmodend %DIFAUXCMD
...
%DIFDELCMD < \begin{minipage}{\hsize}%%%
%DIF < 
\DIFdelend \lstset{frame=single,framexleftmargin=-1pt,framexrightmargin=-17pt,framesep=12pt,linewidth=0.98\textwidth,language=bash}% Set your language (you can change the language for each code-block optionally)
%%% Start your code-block
\DIFdelbegin %DIFDELCMD < \begin{lstlisting}[caption={Example of configuration .ini file to run drone2report},label={lst:config}]
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFmodbegin
\begin{lstlisting}[caption={Example of configuration file to run \textsc{drone2report}.
This takes the form of a standard .ini file, divided in sections (between
square brackets) and supporting comments (using the hash symbol}, label={lst:config},alsolanguage=DIFcode]
[DEFAULT]
%DIF > #these parameters are shared by all sections
...
[DATA image]
%DIF > #this section specifies an input orthomosaic
active=True
...
[TASK indices]
%DIF > #this section tells DRONE2REPORT to compute 
%DIF > #two indices: GLI and HUE
active=True
...
\DIFaddbegin \begin{minipage}{\hsize}%DIF > 
\lstset{frame=single,framexleftmargin=-1pt,framexrightmargin=-17pt,framesep=12pt,linewidth=0.98\textwidth,language=python}%DIF >  Set your language (you can change the language for each code-block optionally)
...
To illustrate how to use \textsc{drone2report} for image processing from drone phenotyping, and to show some potential and useful applications of vegetation indices, we present five case studies: i) thresholding; ii) evolution of vegetation indices over time; iii) \DIFdelbegin \DIFdel{detection of lodging}\DIFdelend \DIFaddbegin \DIFadd{analysis of height}\DIFaddend ; iv) combining multiple sensors and index optimization; v) leveraging a deep neural network for image classification. 
\DIFaddbegin \DIFadd{The code and data to reproduce these case studies can be found at }\url{https://github.com/ne1s0n/paper-drone2report}\DIFadd{.
We emphasize that the presented case studies are not original research work, but mere examples of possible uses of }\textsc{\DIFadd{drone2report}}\DIFadd{.
}\DIFaddend 
...
In this example we compute the Green Leaf Index (GLI\DIFaddbegin \DIFadd{, see Formula~\ref{formula:GLI_index}}\DIFaddend ), whose positive values tend to identify green vegetation (e.g. leaves and stems), while negative values tend to reflect soil/non-vegetation \cite{louhaichi2001spatially}. 
...
To illustrate the removal of background noise, we \DIFaddbegin \DIFadd{first }\DIFaddend use one RGB image of tobacco leaves from the Github repository (\url{https://github.com/tanghaibao/jcvi/wiki/GRABSEEDS}) of the \textsc{GRABSEED} software \cite{tang2024grabseeds} (\DIFdelbegin \DIFdel{see }\DIFdelend Figure~\ref{fig:tobacco-leaves})

\begin{figure}[H]
%    \centering
    \includegraphics[width=0.95\linewidth]{figs/tobacco.png}
    \caption{Example RGB image of tobacco leaves (\textit{Nicotiana tabacum}). A) The original image of the tobacco leaves as downloaded from \url{https://www.dropbox.com/s/is4jrmlmmcfdpdp/test-data.zip?dl=1}; B) The same image with threshold 0.1: the pixels highlighted in turquoise are those that will be used \DIFaddbeginFL \DIFaddFL{by }\textsc{\DIFaddFL{DRONE2REPORT}} \DIFaddendFL for the calculations of the vegetation indices.}
...
%    \centering
    ...
    \caption{Distribution of values for the vegetation index GLI \DIFdelbeginFL \DIFdelFL{(Green Leaf Index) }\DIFdelendFL for: A) the tobacco leaves image; and B) for the barley field plot image}.
    \label{fig:gli}
\end{figure}

In a more realistic case the GLI values clusters from different segments of the image may not be as clear as in the tobacco-leaves example. An example is reported in Figure~\ref{fig:base-field-plot}, panel A: a drone-captured RGB image \DIFaddbegin \DIFadd{(drone: DJI Mavic 3 Enterprise with integrated CMOS camera; resolution: 0.33 cm/pixel; overlap: 80\%; sidelap: 80\%) }\DIFaddend of a single plot from a barley field in Fiorenzuola d’Arda (PC), Italy, $44^{\circ} 55' 27'' N, 9^{\circ}54'47'' E$, taken on March 25th 2024 (``Polyploidbreeding'' research project: \url{https://polyploidbreeding.ibba.cnr.it/}).
...
We see that the number of pixels used decreases with the threshold \DIFdelbegin \DIFdel{used}\DIFdelend \DIFaddbegin \DIFadd{applied}\DIFaddend . Not surprisingly, given that GLI measures the green intensity of pixels in the image, GLI values increase with the threshold, from 0.26 to 0.93: essentially, since Figure~\ref{fig:base-field-plot} (A) is a barley field, we are progressively getting rid of the soil and of the less green parts of the plants. 
...
Note that in these examples, for simplicity, we applied thresholding on the same index \DIFdelbegin \DIFdel{that was }\DIFdelend \DIFaddbegin \DIFadd{to be }\DIFaddend computed. However, it is possible to threshold the image based on one index and then compute one\DIFdelbegin \DIFdel{(or more) }\DIFdelend \DIFaddbegin \DIFadd{, or more, }\DIFaddend other indices on the retained pixels in a single pass. Moreover, it is also possible to specify more complex thresholding, e.g. banding, such as $(\text{GLI} > 0) \wedge (\text{GLI} < 0.8)$. \DIFaddbegin \DIFadd{In all cases, the evaluation of different index values and the selection of the best threshold is not automated in }\textsc{\DIFadd{drone2report}}\DIFadd{.
}\DIFaddend 

\begin{table}[H]
\small
\caption{\DIFdelbeginFL \DIFdelFL{Green }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{An example of reporting table produced by }\textsc{\DIFaddFL{drone2report}}\DIFaddFL{: green }\DIFaddendFL leaf index (GLI) values with different thresholding, as computed by [TASK index]. For each Region Of Interest (as defined in the shapefile) a set of summary statistics is computed for each index (mean, standard deviation, min and max). after\_th: n. of pixels left for the calculation after thresholding. \DIFaddbeginFL \DIFaddFL{Note that with these settings the last column (GLI\_min) strictly reflects the progression of thresholding.}\DIFaddendFL }
...
In this illustration, drone-captured barley-field images from the same experiment as in case study n. 1 were used\DIFaddbegin \DIFadd{; within this field, a different barley variety is sown in each plot}\DIFaddend .
However, instead of using one \DIFdelbegin \DIFdel{single plot }\DIFdelend \DIFaddbegin \DIFadd{single-plot image }\DIFaddend (one single barley variety) \DIFaddbegin \DIFadd{at one single timepoint}\DIFaddend , the images \DIFaddbegin \DIFadd{selected for this case study }\DIFaddend cover 20 plots (20 barley varieties) \DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{over }\DIFaddend 10 flights. \DIFdelbegin \DIFdel{For }\DIFdelend \DIFaddbegin \DIFadd{Flights were performed in Fiorenzuola d'Arda 
(Italy, $44^\circ 55'40.2466'', 009^\circ53'42.4730''$)
between Feb 19, 2024 (first flight on empty field) and June 12, 2024 (tenth flight).
As }\DIFaddend vegetation indices, we use GLI (described before, see \DIFdelbegin \DIFdel{Formula}\DIFdelend \DIFaddbegin \DIFadd{Equation}\DIFaddend ~\ref{formula:GLI_index}) and HUE~\cite{mathieu1998relationships}\DIFaddbegin \DIFadd{, }\DIFaddend defined as:
...
To calculate vegetation indices over multiple images (i.e. successive flights) \DIFdelbegin \DIFdel{for field/area }\DIFdelend using \textsc{drone2report} we can follow two approaches: i) we can add several data sections in the configuration (.ini) file, one for each image, then run \DIFdelbegin \DIFdel{drone2report }\DIFdelend \DIFaddbegin \textsc{\DIFadd{drone2report}} \DIFaddend just once; or ii) iterate over the number of images, automatically generating each .ini file, thus running \textsc{drone2report} as many times as there are images.

Once the chosen vegetation indices are computed for all multiple images, it is possible to plot \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{their }\DIFaddend trends over time to monitor the evolution of the field. 
In the barley \DIFdelbegin \DIFdel{field }\DIFdelend example each plot represents a different variety \DIFaddbegin \DIFadd{(with three repetitions due to experimental design)}\DIFaddend . It is thus possible to \DIFdelbegin \DIFdel{also implement }\DIFdelend \DIFaddbegin \DIFadd{make }\DIFaddend inter-varietal comparisons. This is shown in Figure~\ref{fig:time-curves} (B), where we highlighted two arbitrary varieties (in red and blue). The barley plots \DIFdelbegin \DIFdel{are shown to }\DIFdelend start with moderate green intensity (sparse vegetation), reach a maximum around April to early May (thick vegetation), then decrease toward the end of the growing season (matured yellowish barley in June). This trend matches what we observed in the RGB images (Figure~\ref{fig:time-curves}, A).
The two indexes follow a similar trend, with a peak and a decrease. Interestingly, GLI peaks earlier than HUE, which in turn shows a sharper peak around May. \DIFdelbegin \DIFdel{The indexes also show variability between the different }\DIFdelend \DIFaddbegin \DIFadd{Variability exists between }\DIFaddend barley varieties, \DIFaddbegin \DIFadd{probably }\DIFaddend linked to their different genetic \DIFdelbegin \DIFdel{background}\DIFdelend \DIFaddbegin \DIFadd{make-up}\DIFaddend .
...
\subsection{Case Study N. 3: \DIFdelbegin \DIFdel{Detecting Lodging }\DIFdelend \DIFaddbegin \DIFadd{Analysis of height }\DIFaddend from DEM \DIFdelbegin \DIFdel{Files}\DIFdelend \DIFaddbegin \DIFadd{files}\DIFaddend }
...
In Figure~\ref{fig:dem-height} \DIFdelbegin \DIFdel{, A}\DIFdelend \DIFaddbegin \DIFadd{(A)}\DIFaddend , a hill-shade rendering of a DEM file for the example barley field is reported (part of it), to visualize the 3D structure of the field image. Pixel heights will differ depending on the content, being soil, an erect plant, or a lodged one. 
The evolution of per-plot average heights \DIFaddbegin \DIFadd{over time }\DIFaddend is reported in Figure~\ref{fig:dem-height} (B) for the successive flights. Note that the very first flight is used as reference and thus all pixels are zero by definition.

\begin{figure}[H]
%    \centering
    \includegraphics[width=0.95\linewidth]{figs/dem-height.png}
    \caption{Example .dem file of a barley field (left: flight on 19/04/2024); plant height (metres) calculated from the .dem file by \DIFdelbeginFL \DIFdelFL{drone2report}\DIFdelendFL \DIFaddbeginFL \textsc{\DIFaddFL{drone2report}}\DIFaddendFL , over 10 flights covering the growing season (right\DIFaddbeginFL \DIFaddFL{; the dates for the 10 flights are the same as in Figure~\ref{fig:time-curves}}\DIFaddendFL ).}
    \label{fig:dem-height}
\end{figure}

%DIF > The analysis of DEM files allows us to analyse how the height of crops evolve over time, which might be due to different causes.
After the first flight the heights quickly increase up to the 5th-6th flight (mid-April: the plants were growing). In our barley field trials, \textsc{drone2report} shows a decrease of height after the 7th flight due\DIFaddbegin \DIFadd{, in this case, }\DIFaddend to lodging, consistently with the genetic material examined (old cultivars and landraces) and the adverse weather conditions.
...
\DIFdelbegin \DIFdel{Therefore, we can see that the analysis of DEM files allowed us to detect if and when lodging occurred in our fieldof interest}\DIFdelend \DIFaddbegin \DIFadd{In general, similar height-decrease patterns may be linked to a variety of casues, e.g. they may also indicate the moment when the spike begins to fall over (barley plants are known to “hide” the spike in the vegetation). 
Here we knew there was lodging; however, to be able to detect lodging (and where it occurred in the field) more complex machine-learning models for image segmentation and classification should be implemented, as shown in section~\ref{dl-classification}}\DIFaddend .  

\DIFdelbegin \DIFdel{It is also possible to estimate }\DIFdelend \DIFaddbegin \DIFadd{From the DEM files also }\DIFaddend the per-plot volumes \DIFaddbegin \DIFadd{can be estimated }\DIFaddend using \textsc{drone2report}. Consider that the integral of an area is proportional to the sum of the heights of each pixel. When computing indexes like GLI or HUE the actual result of the computation is a matrix with exactly the same dimension as the ROI (i.e. the plot). It is however possible to select "indexes" that return a single scalar number, e.g. the summation of all values \DIFdelbegin \DIFdel{of the DEM file }\DIFdelend for a plot\DIFdelbegin \DIFdel{. This serves as a numeric }\DIFdelend \DIFaddbegin \DIFadd{, or other custom algebraic operations. For example, adding all height values of a plot (taken from the DEM file) allows to compute a finite }\DIFaddend approximation of the \DIFaddbegin \DIFadd{Riemann integral of the above-ground }\DIFaddend volume, which in turn \DIFdelbegin \DIFdel{could }\DIFdelend \DIFaddbegin \DIFadd{can }\DIFaddend be used to estimate crop yield or biomass.

\subsection{Case Study N. 4: Index Optimization on Images Merged from  Heterogeneous Sensors}

\textsc{drone2report} can transparently import images from different sensors and merge them into a single data structure where all channels of the original images are stacked. For example, it is possible to merge images coming from visible \DIFdelbegin \DIFdel{wavelength }\DIFdelend \DIFaddbegin \DIFadd{wavelengths }\DIFaddend (e.g. RGB images), from \DIFdelbegin \DIFdel{multispectral sensors }\DIFdelend \DIFaddbegin \DIFadd{sensors in the non-visible wavelength }\DIFaddend (e.g. infra-red and near-infra-red images) and from \DIFdelbegin \DIFdel{a thermal camera}\DIFdelend \DIFaddbegin \DIFadd{thermal cameras}\DIFaddend . Differences in image resolutions are managed automatically via \DIFdelbegin \DIFdel{interpolation, and differences }\DIFdelend \DIFaddbegin \DIFadd{cubic interpolation. Differences }\DIFaddend in spatial reference system (SRS) are handled via reprojection \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{of all data into the reference taken from the first input orthomosaic. ROIs are taken from the single shape file (common to all orthomosaics).
}

\DIFaddend The resulting data structure exposes, for each pixel, a number of channels equal to the sum of all channels of the original images. Thus, it is possible to leverage different data sources together and transparently.

In this case study three images are merged: one RGB (three bands\DIFaddbegin \DIFadd{; camera described in Case Study n. 1}\DIFaddend ), one multispectral (ten bands\DIFaddbegin \DIFadd{; camera: DJI Matrice 300 RTK with Micasense Dual Camera System sensor; resolution: 0.83 cm/pixel; overlap: 80\%; sidelap: 80\%}\DIFaddend ) and one thermal (one band\DIFaddbegin \DIFadd{; drone: DJI Mavic 3 Enterprise Thermal with integrated camera; resolution: 1.57 cm/pixel; overlap: 68\%; sidelap: 90\%}\DIFaddend ). All images are of a barley field close to harvesting time. The field is structured in 264 plots, each phenotyped for heading date, i.e. the number of days from sowing when the spike (head) has emerged from the flag leaf sheath in 50\% of the plants in a plot. The aim of of this case study is to optimize a new vegetation index of the form:
...
where $\textbf{p} \in \mathbb{R}^{14}$ is a pixel on the merged 14-bands image. The set of 30 coefficients $[n_0, ..., n_{14}, d_0, ..., d_{14}]$ are to be chosen to maximize the Pearson's correlation between the array of \DIFdelbegin \DIFdel{heading dates and the array of }\DIFdelend per-plot average index values \DIFaddbegin \DIFadd{and the target variable}\DIFaddend . Formula~\ref{optimal_index} is designed as a generalization of the most commonly used indices\DIFaddbegin \DIFadd{. In fact, indices }\DIFaddend such as NDVI, GLI \DIFdelbegin \DIFdel{, VARI , etc.\mbox{%DIFAUXCMD
\cite{henrich2012entwicklung} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{and VARI \mbox{%DIFAUXCMD
\cite{henrich2012entwicklung} }\hspace{0pt}%DIFAUXCMD
can all be derived as special combinations of the $d_i, n_i$ coefficients}\DIFaddend .

This thus becomes a 30-parameters optimization problem, which can be solved via a new custom \textsc{drone2report} task. Internally, the task uses \texttt{Scipy}'s \cite{2020SciPy-NMeth} \textit{optimize.minimize()} function based on the \textit{Nelder-Mead} numerical method \cite{nelder1965simplex}. 
\DIFaddbegin 

\DIFadd{The dataset, consisting of 264 plots and their corresponding heading dates, was split in training set (80\% of the data) and validation set (20\% of the data). }\DIFaddend The results of the optimization process are reported in Figure~\ref{fig:optimization_trajectory}, which shows the optimization trajectory of the custom index in equation~\ref{optimal_index} through all training iterations. \DIFdelbegin \DIFdel{The index was optimized on the merged 14-bands image and, for comparison, on both the visible RGB image and the 10-bands multispectral image, separately. 
}\DIFdelend Additionally, the plot also reports the Pearson's correlation with heading date for the two best performing vegetation indices among the twenty one currently implemented in \textsc{drone2report}, namely NDVI (Normalized Difference Vegetation Index, for multispectral images) and GLI (Green Leaf Index, for RGB images).

\DIFdelbegin %DIFDELCMD < \begin{figure}[H]
%DIFDELCMD < %%%
%DIF <     \centering
    %DIFDELCMD < \includegraphics[width=1\linewidth]{figs/trajectories.png}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Optimization trajectories through training cycles. The custom index described in Formula~\ref{optimal_index} was optimized for maximal Pearson's correlation between the vector of its per-plot averages and the phenotype }\textit{\DIFdelFL{heading date}}%DIFAUXCMD
\DIFdelFL{. Solid lines show three optimization trajectories on different input images, namely RGB, 10-bands multispectral, and a 14-bands merged image containing RGB, multispectral and temperature data. For reference, the two best performing indices (NDVI and GLI) are also reported.
    }%DIFDELCMD < \label{fig:optimization_trajectory}
%DIFDELCMD <     %%%
}
%DIFAUXCMD
...
%DIFDELCMD < %%%
\DIFdel{The results show that it is possible to optimize a custom vegetation index on a specific drone dataset and beat the correlations of standard indices. The optimization process is iterative and stochastic, so different images required different amounts of iteration before converging. Optimizing on the RGB image required little more than two hundred iterations}\DIFdelend \DIFaddbegin \DIFadd{Notice how the correlation on the validation set trails below the values computed on the training set. At some point, just before iteration 250, the correlation on the validation set drops}\DIFaddend , while on the \DIFdelbegin \DIFdel{14-bands merged image itrequired 1148 iterations. These numbers would change if the process were repeated and are influenced by the initialization process, which assigns random values to the parameters. However, in general a longer optimization process is expected the more parameters are to be tuned. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The indices obtained with this case study are likely overfitting, at least partially. A focused, future study, could estimate the fraction of overfitting and apply regularization techniques to ensure good generalization of the results}\DIFdelend \DIFaddbegin \DIFadd{training set continues to rise. This is expected, and it's a clear symptom of overfitting. This can easily be contrasted by a number of techniques, e.g. Early Stopping~\mbox{%DIFAUXCMD
\cite{yao2007early}}\hspace{0pt}%DIFAUXCMD
, that can intercept the sudden drop on the validation set and stop the training process. Alternatively, the set of parameters maximizing correlation on the validation set can be extracted from the training log. In this second case the training will continue to run until the optimizer's convergence criteria are met}\DIFaddend .

\DIFaddbegin \begin{figure}[H]
%DIF >     \centering
    \includegraphics[width=1\linewidth]{figs/trajectories.png}
    \caption{\DIFaddFL{Optimization trajectories through training cycles. The custom index described in Formula~\ref{optimal_index} was optimized for maximal Pearson's correlation between the vector of its per-plot averages and the phenotype }\textit{\DIFaddFL{heading date}} \DIFaddFL{(here acting as target variable). The color indicates if the values refers to the correlation on the training or validation set. For reference, the two best performing indices (NDVI and GLI) are also reported.
    }\label{fig:optimization_trajectory}
    }
\end{figure}

\DIFaddend \subsection{Case Study N. 5: Deep Learning for Image Classification}
\DIFaddbegin \label{dl-classification}
\DIFaddend 

\textsc{drone2report} can be used to feed georeferenced data into a trained deep-learning model and to collect inference results with minimal user-provided code. For this case study, we used UAV imagery from a publicly available dataset developed for weed detection in soybean \cite{Peccia_2018}. \DIFaddbegin \DIFadd{This is a four-classes (soybean, broadleef weed, grass, bare soil) classification problem, comprising a 15,926 images dataset.
}

\DIFaddend As a preliminary step, we trained a deep neural network based on the well-known ResNet50 architecture \cite{he2016deep}, as implemented in the TensorFlow/Keras framework \cite{chollet2015keras}. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend The complete training procedure, including the modifications applied to the base ResNet50 architecture and the strategies adopted for early stopping and learning-rate scheduling, is reported in a supplementary notebook to ensure reproducibility. Out of \DIFdelbegin \DIFdel{the 15,926 images available in the dataset}\DIFdelend \DIFaddbegin \DIFadd{all available images}\DIFaddend , forty images (ten per class) were retained and completely isolated from the training process to form an independent test set. The remaining images were split between training (70\%\DIFaddbegin \DIFadd{, 11,120 images}\DIFaddend ) and validation (30\%\DIFaddbegin \DIFadd{, 4766 images}\DIFaddend ) subsets. The corresponding training trajectories are reported in Figure~\ref{fig:classification_resnet}.
...
\DIFaddbegin \DIFadd{Figure~\ref{fig:classification_resnet} shows that the training saturates after 10-12 training epochs, and shows no sign of overfitting (loss on training set and validation set are very similar).
}

\DIFaddend Once training was complete, the trained network object (comprising both the model architecture and the optimized weights) was retained\DIFdelbegin \DIFdel{and made available as supplementary material}\DIFdelend . To enable \textsc{drone2report} to leverage the trained network, a custom [TASK] script was implemented to (1) load the trained model, (2) preprocess the raster region-of-interest (ROI) data to match the network input requirements, (3) execute the network in inference mode, and (4) collect the prediction results into an output file. The custom task \DIFdelbegin \DIFdel{comprised }\DIFdelend \DIFaddbegin \DIFadd{comprises }\DIFaddend approximately eighty lines of code, including inline comments and informational messages to guide the user through the execution steps. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Given the demonstrative nature of the task, the model achieved a perfect classification score on the test set}\DIFdelend \DIFaddbegin \DIFadd{The network was run on the test set (forty images, ten per class, not involved during the network training), on which it achieved perfect classification accuracy. This simulates a real world application where an externally trained model is applied on incoming data to guide management decisions}\DIFaddend .
...
\subsection{\DIFdelbegin \DIFdel{State of the Art }\DIFdelend \DIFaddbegin \DIFadd{State-of-the-Art free software solutions, }\DIFaddend and Positioning of \textsc{drone2report}}
...
\textit{QGIS} (\url{https://qgis.org/}, \cite{moyroud2018introduction}) is a free and open-source desktop geographic information system (GIS) suite. It allows users to create, edit, visualize, analyze, and publish geospatial information. In particular, QGIS provides a Processing Modeler \cite{dobesova2020evaluation} for chaining algorithms and running them in batch, which can partially address reproducibility. In practice, model portability, parameter/version management, and experiment-level reporting remain challenging, especially across groups and \DIFdelbegin \DIFdel{compute }\DIFdelend \DIFaddbegin \DIFadd{computer }\DIFaddend environments.
...
\DIFdelbegin \DIFdel{Moreover, the software is open source and freely (and easily) customizable.
}\DIFdelend %DIF > Moreover, the software is open source and freely (and easily) customizable.
...
In short, while \textit{ODM/WebODM} specializes in \DIFdelbegin \DIFdel{map making}\DIFdelend \DIFaddbegin \DIFadd{producing sophisticated digital outputs from raw UAV-imagery data}\DIFaddend , \textit{OTB} and \textit{QGIS} provide low-level building blocks, \textit{GRABSEEDS} and \textit{BerryPortraits} occupy specific application niches, \textit{GRID} and \textit{PREPs} offer flexible segmentation functionalities, and \textit{PlantCV} and \textit{FIELDimageR} target image-centric or R-centric workflows, \textsc{drone2report} contributes a domain-specific, geospatial, configuration-driven bridge from orthomosaics to plot-level, multi-sensor, repeatable reports tailored to field phenotyping. Moreover, \DIFdelbegin \DIFdel{drone2report }\DIFdelend \DIFaddbegin \textsc{\DIFadd{drone2report}} \DIFaddend can easily integrate heterogeneous predictive models, such as deep neural networks, with minimal code adaptation.
...
\DIFdelbegin \DIFdel{In conclusion, }\DIFdelend \DIFaddbegin \section{\DIFadd{Conclusions}}
\label{conclusions}

\DIFaddend \textsc{drone2report} is a new software tool designed to support repeatable, scalable, and multi-sensor UAV data analysis in agricultural field trials\DIFdelbegin \DIFdel{, and is going to be of great use and help for the scientific and technical community in precision agriculture and beyond. By focusing on vegetation index calculation, ROI-level analytics, }\DIFdelend \DIFaddbegin \DIFadd{. It is designed to bridge the gap between orthomosaic creation and actual agriculture applications, a gap typically filled with custom, non-sharable software solutions. Moreover, }\textsc{\DIFadd{drone2report}} \DIFadd{is designed around reproductibility and modularity, }\DIFaddend and \DIFdelbegin \DIFdel{reproducible reporting, it fills a critical methodological gap between raw imagery and downstream statistical analysis}\DIFdelend \DIFaddbegin \DIFadd{it's easily expandable to integrate external models}\DIFaddend . Its design makes it a valuable asset not only for researchers in precision agriculture, but also for broader applications such as forestry, land-use monitoring, and environmental surveys.
...
%DIF > %% 36
\DIFaddbegin \bibitem[\protect\citeauthoryear{Park et~al.}{2023}]{park2022deep}
\begin{barticle}
\bauthor{\bsnm{Park}, \binits{J.}}\DIFadd{,
}\bauthor{\bsnm{Cho}, \binits{Y.}}\DIFadd{,
}\bauthor{\bsnm{Kim}, \binits{S.}}\DIFadd{:
}\batitle{Deep learning-based UAV image segmentation and inpainting for generating vehicle-free orthomosaic}\DIFadd{.
}\bjtitle{International Journal of Applied Earth Observation and Geoinformation}
\bvolume{115}\DIFadd{(}\bissue{}\DIFadd{),
}\bfpage{103111}
\DIFadd{(}\byear{2022}\DIFadd{)
}\end{barticle}
\endbibitem
...
%DIF > %% 37
\DIFaddbegin \bibitem[\protect\citeauthoryear{Sch\"{o}nberger et~al.}{2016}]{schoenberger2016sfm}
\begin{bbook}
\bauthor{\bsnm{Sch\"{o}nberger}, \binits{J.L.}}\DIFadd{,
}\bauthor{\bsnm{Frahm}, \binits{J.M.}}\DIFadd{:
}\batitle{Structure-from-Motion Revisited}\DIFadd{.
}\bjtitle{Conference on Computer Vision and Pattern Recognition (CVPR)}
%DIF > \bvolume{2}(\bissue{1}),
%DIF > \bfpage{14}
\DIFadd{(}\byear{2016}\DIFadd{)
}\end{bbook}
\endbibitem
...
(\byear{1965}\DIFaddbegin \DIFadd{)
}\end{barticle}
\endbibitem

%DIF > %% 36
\bibitem[\protect\citeauthoryear{Yao et~al.}{2007}]{yao2007early}
\begin{barticle}
\bauthor{\bsnm{Yao}, \binits{Y.}}\DIFadd{,
}\bauthor{\bsnm{Rosasco}, \binits{L.}}\DIFadd{,
}\bauthor{\bsnm{Caponnetto}, \binits{A.}}\DIFadd{,
}\batitle{On early stopping in gradient descent learning}\DIFadd{.
}\bjtitle{Constructive approximation}
\bvolume{26}\DIFadd{(}\bissue{2}\DIFadd{),
}\bfpage{289}\DIFadd{--}\blpage{315}
\DIFadd{(}\byear{2007}\DIFaddend )
\end{barticle}
\endbibitem
