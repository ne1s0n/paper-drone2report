"""
	...
	named array (implemented as a dictionary) as output. 
	The function uses 80% of the ROIs as training set and the
	remaining as validation set.
...
class optimize_index_train_val(Task):
	...
	def _set_x0(self, nchan, logfile):
		...
		if os.path.isfile(logfile):
			...
			with open(logfile, newline='') as csvfile:
				...
				for row in re:
					#each row contains: timestamp, correlation_train, correlation_val, <parameters>
					#we just need to remove the first three elements
					x0 = row[3:]
	...
	def run(self, dataset):
		...
		if not os.path.isfile(outfile):
			new_row = {'time' : None, 'correlation_train' : None, 'correlation_val' : None}
		...
		ids = true_phenos[self.config['id_col']].astype(str).values
		np.random.seed(42)
		perm = np.random.permutation(len(ids))
		split_idx = int(0.8 * len(ids)) #an 80/20 split

		#the indexes of the samples for the two sets
		train_idx = perm[:split_idx]
		val_idx   = perm[split_idx:]

		#the actual phenotypes
		train_phenos = true_phenos.iloc[train_idx].reset_index(drop=True)
		val_phenos   = true_phenos.iloc[val_idx].reset_index(drop=True)
		
		#a little interface
		print('Training samples: ' + str(len(train_phenos)))
		print('Validation samples: ' + str(len(val_phenos)))
		...
		extraconf = {
			...
			'val_phenotypes' : val_phenos,
			'idx_train' : train_idx,
			'idx_val' : val_idx,
			'outfile' : outfile
		}
		
		#ready to optimize (training mode)
...
def mycallback(xk):
	...
	#evaluating the funcion at the current solution on both sets
	sol_train = compute_index(xk, mode='train')
	sol_val = compute_index(xk, mode='val')
	
	#taking notes
	new_row = {
		'time' : str(datetime.datetime.now()), 
		'correlation_train' : -sol_train, #saving the actual correlation, not it's negative
		'correlation_val'   : -sol_val    #both train and validation sets
	}
	for i in range(len(xk)):
		new_row['param_' + str(i)] = xk[i]

	#saving to an outfile
	with open(outfile, "a", newline="") as f:
		w = csv.DictWriter(f, new_row.keys())
		w.writerow(new_row)

def compute_index(current_weights, mode='train'):
	'''This function computes the index, using the passed weight, on
	either the train set or the validation set. Only valid values
	for mode are 'train' and 'val'
	'''
	
	#a bit of interface
	print(' - invoked compute_index, mode:' + mode)
	...
	if mode == 'train':
		phenotypes = extraconf['train_phenotypes']
		plot_idx = extraconf['idx_train']
	elif mode == 'val':
		phenotypes = extraconf['val_phenotypes']
		plot_idx = extraconf['idx_val']
	else:
	...
	#for each shape in the target dataset
	#for i in range(len(geometry_columns)): #this could be parallelized
	for i in plot_idx: #this loop could be parallelized
